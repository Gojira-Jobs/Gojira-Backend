...
exports.superauth = (req,res,next)=> {
    ...
    else if(!req.headers.username)  res.status(401).send({'status': 0,'data':'Username empty'});
    else {
         var token = req.headers.token;
         var userid = req.headers.username.toLowerCase();
...
exports.admin = (req,res,next)=>{
    ...
    if(!req.headers.token)  res.status(401).send({'success':false,'msg':'unauthorized user'});
    else if(!req.headers.username)  res.status(401).send({'success':false,'msg':'Username empty'});
    else {
         var token = req.headers.token;
         var userid = req.headers.username.toLowerCase();
         console.log('Token and Username: '+ token,userid);
